...
function StatusChart({ taskDetailData, setTaskDetail }) {
  const data = {
    ...
    datasets: [
      {
        label: "APPROVED",
        backgroundColor: findDataProperty(taskDetailData, "approved", "color"),
        data: [findDataProperty(taskDetailData, "approved", "val")],
        borderSkipped: "left",
  ...
  statusChartOptions.onClick = (event, element, chart) =>
    chartOnClick(event, element, chart, setTaskDetail, taskDetailData);
